...
class Alt_Dbo {
    ...
    public function count($data = array(), $returnsql = false) {
        // sql query
        $sql = "select count(*) as numofrow from " . $this->get_tablename() . $this->get_where($data);
    ...
    public function insert_multi($data, $returnsql = false){
        ...
        // add entry time and entry user if exist
        if($fields["entrytime"] !== null)   $data[0]["entrytime"] = $data[0]["entrytime"] != "" ? $data[0]["entrytime"] : time();
        if($fields["entryuser"] !== null){
            $userdata = System_Auth::get_userdata();
        ...
        foreach($data as $i => $item){
            ...
            // add entry time and entry user if exist
            if($fields["entrytime"] !== null)   $item["entrytime"] = $item["entrytime"] != "" ? $item["entrytime"] : time();
            if($fields["entryuser"] !== null){
                $userdata = System_Auth::get_userdata();
